...
namespace BezierSolution
{
	...
	public class BezierSpline : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			{
				CompLength = GetLengthApproximately(0f,1f);

				for (int i = 0; i < endPoints.Count; i++)
				{
					FindNearestPointTo(endPoints[i].transform.position, out endPoints[i].splineT, 100f);
				}
			}
		}
		...
		public Vector3 FindNearestPointTo(Vector3 worldPos, out float normalizedT, float accuracy = 100f, float posRef = -1f)
		{
			///TODO
			///guardar a posiçao anterior e comparar com a posiçao nova e usar isso pa encontrar 
			///melhor os T (evita skips na pista)		
			...
			for(int i = 0; i < endPoints.Count; i++)
            {
				float dist = Vector3.Distance(worldPos, endPoints[i].transform.position);
			...
			int chosenI2 = chosenI + 1;

			if (chosenI2 == endPoints.Count)
            {
				chosenI2 = 0;
            }

			int chosenIB = chosenI - 1;

			if (chosenIB < 0)
            {
				chosenIB = endPoints.Count - 1;
            }

			float step = AccuracyToStepSize( accuracy );

			float minDistance = Mathf.Infinity;
			for( float i = endPoints[chosenIB].splineT; i < endPoints[chosenI2].splineT; i += step )
			{
				Vector3 thisPoint = GetPoint( i );
